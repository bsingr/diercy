...
Diercy.Core = function(data){
	...
	this.createRoutes = function() {
		...
		Diercy.Utils.each(myDiercyCore.nodes, function(i){
			var node = this;
			var newRoute = [];
			myDiercyCore.createRoute(node, newRoute);
		});
		...
		Diercy.Utils.each(myDiercyCore.routes, function(i){
			...
			Diercy.Utils.each(route, function(ii){
				...
				Diercy.Utils.each(longestRoute, function(level){
					var node2 = this;
					if (node1 == node2) {
						expandedRoute.push(node1);
					} else {
						expandedRoute.push(null);
					}
				});
			});
			route = expandedRoute;
	...
	this.createRoute = function(node, tempRoute) {
		var myDiercyCore = this;
		console.log(node);
		if (!node.hasRoute) {
			tempRoute.push(node.id); // append self
			if (node.childs) { // not end-node
				Diercy.Utils.each(node.childs, function(i){
					...
					myDiercyCore.createRoute(childNode, tempRoute);			
				});
			} else { // end-node
				myDiercyCore.routes.push(tempRoute);
				if (tempRoute.length > myDiercyCore.routes[myDiercyCore.indexOfLongestRoute].length) {
					myDiercyCore.indexOfLongestRoute = myDiercyCore.routes.length;
				}
			}
			node.hasRoute = true;
		}
